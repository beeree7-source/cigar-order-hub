...
const resolveInvoiceItems = (rawItems = []) => {
  return new Promise((resolve, reject) => {
    ...
    const missingPriceProductIds = [
      ...new Set(
        normalizedItems
          .filter((item) => (item.price === undefined || item.price === null) && item.productId)
          .map((item) => Number(item.productId))
      )
    ];

    if (missingPriceProductIds.length === 0) {
      return resolve(normalizedItems);
    }

    const placeholders = missingPriceProductIds.map(() => '?').join(',');
    db.all(
      ...
      (err, products) => {
        ...
        const resolvedItems = normalizedItems.map((item) => {
          ...
          return {
            ...item,
            price: priceByProductId.get(Number(item.productId)) ?? 0
          };
        });
        ...
        db.get(
          ...
          (err, order) => {
            ...
            try {
              ...
              return res.status(400).json({ error: 'Order items are invalid JSON' });
            }

            resolveInvoiceItems(parsedItems)
              .then((items) => {
                const hasInvalidItems = items.some(
                  (item) => !Number.isFinite(item.quantity) || item.quantity <= 0 || !Number.isFinite(item.price)
                );

                if (hasInvalidItems) {
                  return res.status(400).json({ error: 'Order items are invalid for invoice calculation' });
                }
              ...
              .catch((resolveError) => {
                console.error('Error resolving invoice item pricing:', resolveError);
                return res.status(500).json({ error: 'Failed to resolve invoice item pricing' });
              });
          }
        );
      }
