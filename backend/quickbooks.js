...
const connectQuickBooks = async (req, res) => {
  try {
    ...
    const mockAuthUrl = `https://appcenter.intuit.com/connect/oauth2?client_id=MOCK_CLIENT_ID&redirect_uri=${encodeURIComponent(process.env.QUICKBOOKS_REDIRECT_URI || 'http://localhost:10000/api/quickbooks/callback')}&response_type=code&scope=com.intuit.quickbooks.accounting&state=${csrfToken}`;
