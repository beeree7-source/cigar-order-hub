...
### Accounting Hub Quick Start

Accounting Hub is currently paused by default. QuickBooks automation paths remain available for `supplier`, `retailer`, and `admin` users.
...
3. **Run a quick API smoke test (paused mode)**
```bash
# Replace TOKEN with the JWT returned from login
curl -H "Authorization: Bearer TOKEN" http://localhost:10000/api/accounting/status
curl -X POST -H "Authorization: Bearer TOKEN" http://localhost:10000/api/accounting/sync
curl -X POST -H "Authorization: Bearer TOKEN" http://localhost:10000/api/accounting/sync-orders
curl -X POST -H "Authorization: Bearer TOKEN" http://localhost:10000/api/accounting/sync-customers
```

**PowerShell API Smoke Test Example (Windows):**
```powershell
...
try {
  Invoke-RestMethod -Method Get -Uri "$base/api/accounting/status" -Headers $headers
} catch {
  $_.Exception.Response.StatusCode.value__
}
Invoke-RestMethod -Method Post -Uri "$base/api/accounting/sync" -Headers $headers -ContentType 'application/json' -Body '{}'
Invoke-RestMethod -Method Post -Uri "$base/api/accounting/sync-orders" -Headers $headers -ContentType 'application/json' -Body '{}'
Invoke-RestMethod -Method Post -Uri "$base/api/accounting/sync-customers" -Headers $headers -ContentType 'application/json' -Body '{}'
```

4. **Open web UI pages**
- Supplier QuickBooks: `http://localhost:3000/suppliers/quickbooks`
- Retailer QuickBooks: `http://localhost:3000/retailers/quickbooks`
- Supplier/Retailer messaging: `http://localhost:3000/messages` (approved supplier-retailer pairs only)

5. **Invoice import flow**
- Send multipart form field `invoiceFile` to:
  - `POST /api/accounting/invoices/upload`
- This endpoint stays active while Accounting Suite is paused.
- Recommended invoice artifacts: `.pdf`, `.csv`, `.xlsx`, `.txt`

6. **Invoice generation troubleshooting**
- `POST /api/invoices/order/:id/generate` supports order items without inline `price` fields.
- When `price` is missing, backend resolves pricing from `products` via `productId`/`product_id`.
- If a product is missing from `products`, its resolved price is `0`, so verify product data if totals look low.
- If an order already has an invoice, generation returns: `Invoice already exists for this order`.

**PowerShell Invoice Upload Example (Windows):**
```powershell
$base = 'http://localhost:10000'
$loginBody = @{ email='john@example.com'; password='password123' } | ConvertTo-Json
$login = Invoke-RestMethod -Method Post -Uri "$base/api/auth/login" -ContentType 'application/json' -Body $loginBody
...
# Example invoice artifact
$invoicePath = Join-Path $PWD 'sample-invoice-upload.txt'
'invoice upload test payload' | Set-Content -Path $invoicePath -Encoding UTF8

Invoke-RestMethod -Method Post \
  -Uri "$base/api/accounting/invoices/upload" \
  -Headers $headers \
  -Form @{ invoiceFile = Get-Item $invoicePath; invoiceId = '1' }

Remove-Item $invoicePath -Force
```

**PowerShell Full Accounting E2E Example (Windows):**
```powershell
$base = 'http://localhost:10000'
$loginBody = @{ email='john@example.com'; password='password123' } | ConvertTo-Json
$login = Invoke-RestMethod -Method Post -Uri "$base/api/auth/login" -ContentType 'application/json' -Body $loginBody
...
$genBody = @{ discount=0; taxRate=0.08; paymentTerms='Net 30'; notes='README E2E test' } | ConvertTo-Json
...
# 3) Mark invoice paid and verify
...
See [API Documentation](./API_DOCUMENTATION.md) for full request/response examples.
...
- **[Communication API Documentation](./COMMUNICATION_API_DOCUMENTATION.md)** - Contacts, messages, and call-log endpoints for approved supplier-retailer communication
- **[Communication Setup Guide](./COMMUNICATION_SETUP_GUIDE.md)** - Startup steps and smoke tests for messaging/calls
- **[Supplier API Documentation](./SUPPLIER_API_DOCUMENTATION.md)** - Detailed supplier management endpoints
...
QUICKBOOKS_REDIRECT_URI=http://localhost:10000/api/quickbooks/callback
QUICKBOOKS_ENVIRONMENT=sandbox

# Accounting Suite Toggle (default paused)
