...
export default function SupplierApplicationsPage() {
  ...
  const loadApplications = async () => {
    try {
      const response = await fetch('http://localhost:10000/api/wholesaler/retailer-applications', {
  ...
  const handleApprove = async (applicationId: number) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:10000/api/wholesaler/applications/${applicationId}/approve`,
        {
          method: 'POST',
          headers: {
  ...
  const handleDeny = async (applicationId: number, reason: string) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:10000/api/wholesaler/applications/${applicationId}/deny`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-8">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <Link href="/wholesaler" className="text-blue-400 hover:text-blue-300 mb-4 inline-block">
            ← Back to Dashboard
          </Link>
