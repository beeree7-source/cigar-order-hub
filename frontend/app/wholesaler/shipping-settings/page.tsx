...
export default function SupplierShippingSettingsPage() {
    ...
    const fetchAccounts = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:10000/api/wholesaler/${user.id}/shipping/accounts`,
          {
            headers: {
  ...
  const handleAddAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:10000/api/wholesaler/${supplierId}/shipping/account`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      const listResponse = await fetch(
        `http://localhost:10000/api/wholesaler/${supplierId}/shipping/accounts`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  const handleDeleteAccount = async (accountId: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:10000/api/wholesaler/${supplierId}/shipping/account/${accountId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
