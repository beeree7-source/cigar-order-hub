...
export default function SupplierSubscriptionPage() {
  ...
  useEffect(() => {
    ...
    if (userObj.role !== 'supplier' && userObj.role !== 'admin') {
      router.push('/');
      return;
    }
  ...
  const loadTiers = async (token: string) => {
    try {
      setError('');
      const response = await fetch('http://localhost:10000/api/supplier-subscription/tiers', {
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to load supplier subscription tiers');
      }

      const data = await response.json();
      setTiers(data.tiers || []);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load supplier subscription tiers');
    }
  };

  const loadCurrentSubscription = async (token: string) => {
    try {
      const response = await fetch('http://localhost:10000/api/supplier-subscription/current', {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const handleSelectPlan = async (tier: SubscriptionTier) => {
    ...
    try {
      ...
      if (!currentSubscription) {
        response = await fetch('http://localhost:10000/api/supplier-subscription/subscribe', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        response = await fetch('http://localhost:10000/api/supplier-subscription/change-tier', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
          ...
          body: JSON.stringify({
            ...
            reason: 'Changed from supplier subscription UI'
          })
        });
      }
  ...
  const handleCancelPlan = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:10000/api/supplier-subscription/cancel', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          immediate: false,
          reason: 'Cancelled from supplier subscription page'
        })
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold text-white mb-2">Supplier SaaS Plans</h1>
            <p className="text-gray-400">Choose the plan that fits your supplier growth stage.</p>
          </div>
          <Link
            href="/suppliers"
            ...
            {sortedTiers.map((tier) => (
              <div key={tier.id} className="bg-gray-800 border border-gray-700 rounded-xl p-6">
                ...
                <ul className="space-y-2 text-sm text-gray-300 mb-6">
                  <li>Products: {formatLimit(tier.max_products)}</li>
                  <li>Retailers: {formatLimit(tier.max_retailers)}</li>
                  <li>Team Members: {formatLimit(tier.max_team_members)}</li>
