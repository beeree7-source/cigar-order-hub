...
export default function ApplyToSuppliersPage() {
  ...
  const loadData = async () => {
    try {
      const [suppliersRes, licensesRes] = await Promise.all([
        fetch('http://localhost:10000/api/retailer/all-suppliers', {
  ...
  const handleSelectSupplier = async (supplier: Supplier) => {
    ...
    if (supplier.status === 'none' && supplier.has_applications) {
      try {
        const response = await fetch(`http://localhost:10000/api/wholesaler/id/${supplier.supplier_id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });

        if (response.ok) {
          const supplierAppsRes = await fetch(`http://localhost:10000/api/wholesaler/${supplier.supplier_id}/applications`, {
            headers: { Authorization: `Bearer ${token}` }
          });
  ...
  const handleSubmitApplication = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:10000/api/retailer/applications', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-8">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <Link href="/retailer" className="text-blue-400 hover:text-blue-300 mb-4 inline-block">
            ← Back to Dashboard
          </Link>
